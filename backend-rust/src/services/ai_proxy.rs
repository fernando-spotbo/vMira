...
pub fn stream_ai_response(
    ...
    if has_user_system_msg {
        ...
        // CLI or direct API — detect which by checking for CLI tool XML in messages
        let is_cli = messages.iter().any(|m| m.content.contains("<available-deferred-tools>"));

        let prompt = if is_cli {
            // CLI — coding assistant personality
            format!(
                "You are vMira, an AI coding assistant powered by model {model}.\n\
                \n\
